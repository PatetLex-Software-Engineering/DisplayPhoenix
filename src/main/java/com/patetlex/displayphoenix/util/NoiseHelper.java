...
public class NoiseHelper {
    ...
    public static BufferedImage getNoiseImage(int width, int height, double zoom, int x, int y, long seed) {
        OpenSimplexNoise noise = new OpenSimplexNoise(seed);
        BufferedImage image = new BufferedImage(width, height, BufferedImage.TYPE_INT_RGB);
        for (int i = 0; i < width; i++) {
            for (int j = 0; j < height; j++) {
                float val = (float) noise.eval((i + x) / zoom, (j + y) / zoom);
                val += 1;
                image.setRGB(i, j, new Color(Math.round(val * 127.5F), Math.round(val * 127.5F), Math.round(val * 127.5F)).getRGB());
            }
        }
        return image;
    }
